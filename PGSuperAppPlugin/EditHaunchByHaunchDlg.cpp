...
void CEditHaunchByHaunchDlg::DoDataExchange(CDataExchange* pDX)
{
   ...
   if (m_bNeedsGroupTabs && pgsTypes::hltAlongSegments == GetHaunchLayoutType())
   {
...
{
   pgsTypes::HaunchInputLocationType oldInputType = m_HaunchInputLocationType;
...
void CEditHaunchByHaunchDlg::OnHaunchDepthTypeChanged()
{
   UpdateActiveControls();
}

void CEditHaunchByHaunchDlg::NormalizeHaunchDepths(std::vector<Float64>& rDepths,pgsTypes::HaunchInputDistributionType disttype,const CBridgeDescription2* pBridgeDesc)
{
   if (rDepths.empty())
   {
      // No haunch input. Default to fillet across all depths
      Float64 fillet = pBridgeDesc->GetFillet();
      rDepths.assign((int)disttype, fillet);
   }
   else if (rDepths.size() < disttype)
   {
      // vector is smaller than required values. assign last value to rest of vector
      Float64 default = rDepths.back();
      int remain = disttype - (int)rDepths.size();
      for (int i = 1; i <= remain; i++)
      {
         rDepths.push_back(default);
      }
   }
   else if (rDepths.size() > disttype)
   {
      rDepths.resize(disttype);
   }
}

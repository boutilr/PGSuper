...
CXBeamDisplayObjectFactory::CXBeamDisplayObjectFactory()
{
...
std::shared_ptr<WBFL::DManip::iDisplayObject> CXBeamDisplayObjectFactory::Create(CLIPFORMAT cfFormat, COleDataObject* pDataObject) const
{
   if ( cfFormat == CXBeamSectionCutDisplayImpl::ms_Format )
   {
      auto doSectionCut = WBFL::DManip::PointDisplayObject::Create();

      doSectionCut->SetSelectionType(WBFL::DManip::SelectionType::All);

      auto pDisplayImpl = std::make_shared<CXBeamSectionCutDisplayImpl>();

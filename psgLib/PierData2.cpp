///////////////////////////////////////////////////////////////////////
// ConnectionLibraryEntryuper - Prestressed Girder SUPERstructure Design and Analysis
...
CPierData2::CPierData2()
{
   ...
   for ( int i = 0; i < 2; i++ )
   {
      ...
      m_EndDistanceMeasurementType[i]   = ConnectionLibraryEntry::EndDistanceMeasurementType::FromBearingNormalToPier;

      m_GirderBearingOffset[i]          = WBFL::Units::ConvertToSysUnits(1.0,WBFL::Units::Measure::Feet);
      m_BearingOffsetMeasurementType[i] = ConnectionLibraryEntry::BearingOffsetMeasurementType::NormalToPier;

      m_DiaphragmHeight[i] = 0;
      m_DiaphragmWidth[i] = 0;
      m_DiaphragmLoadType[i] = ConnectionLibraryEntry::DiaphragmLoadType::DontApply;
...
HRESULT CPierData2::Save(IStructuredSave* pStrSave,std::shared_ptr<IEAFProgress> pProgress)
{
   HRESULT hr = S_OK;
...

   {
      ...
      if (m_DiaphragmLoadType[pgsTypes::Back] == ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtBearingCenterline)
      {
         pStrSave->put_Property(_T("DiaphragmLoadType"),CComVariant(_T("ApplyAtBearingCenterline")));
      }
      else if (m_DiaphragmLoadType[pgsTypes::Back] == ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtSpecifiedLocation)
      {
      ...
      else if (m_DiaphragmLoadType[pgsTypes::Back] == ConnectionLibraryEntry::DiaphragmLoadType::DontApply)
      {
...

   {
      ...
      if (m_DiaphragmLoadType[pgsTypes::Ahead] == ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtBearingCenterline)
      {
         pStrSave->put_Property(_T("DiaphragmLoadType"),CComVariant(_T("ApplyAtBearingCenterline")));
      }
      else if (m_DiaphragmLoadType[pgsTypes::Ahead] == ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtSpecifiedLocation)
      {
         pStrSave->put_Property(_T("DiaphragmLoadType"),CComVariant(_T("ApplyAtSpecifiedLocation")));
         pStrSave->put_Property(_T("DiaphragmLoadLocation"),CComVariant(m_DiaphragmLoadLocation[pgsTypes::Ahead]));
      }
      else if (m_DiaphragmLoadType[pgsTypes::Ahead] == ConnectionLibraryEntry::DiaphragmLoadType::DontApply)
      {
         pStrSave->put_Property(_T("DiaphragmLoadType"),CComVariant(_T("DontApply")));
      }
      else
...
HRESULT CPierData2::Load(IStructuredLoad* pStrLoad,std::shared_ptr<IEAFProgress> pProgress)
{
   USES_CONVERSION;
   CHRException hr;
   ...
   try
   {
      ...
      if ( 12 < version && version < 20)
      {
         ...
         if ( SUCCEEDED(pStrLoad->get_Property(_T("CantileverLength"),&var)) )
         {
            ...
            m_EndDistanceMeasurementType[face] = ConnectionLibraryEntry::EndDistanceMeasurementType::FromBearingNormalToPier;

            m_GirderBearingOffset[face] = 0; // bearing offset is at the reference line
            m_BearingOffsetMeasurementType[face] = ConnectionLibraryEntry::BearingOffsetMeasurementType::NormalToPier;
         }
      }
      ...
      if ( 13 < version )
      {
         ...
         if ( m_PierModelType == pgsTypes::pmtPhysical )
         {
            if ( version < 15 )
            {
               // Version 15 and Ec was during beta-development of ConnectionLibraryEntryuper/ConnectionLibraryEntryplice
            ...
            if ( 15 < version )
            {
               ...
               // NOTE: during the original implementation of v3 (ConnectionLibraryEntryplice)
               ...
               // flexible footing systems. However, it turns out that the bridge
      ...
      else
      {
         ...
         if (m_pBridgeDesc->GetBearingType() != pgsTypes::brtBridge)
         {
            ...
            {
               // This was a bug in ConnectionLibraryEntryuper version 4.0.10. The program could write out a value of cnt==1, but then have no data to read.
               ATLASSERT(cnt == 1);
               hr = pStrLoad->EndUnit();
            }
         }
...

      {
         ...
         {
            m_DiaphragmLoadType[pgsTypes::Back] = ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtBearingCenterline;
         }
         else if (tmp == _T("ApplyAtSpecifiedLocation"))
         {
            m_DiaphragmLoadType[pgsTypes::Back] = ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtSpecifiedLocation;
         ...
         {
            m_DiaphragmLoadType[pgsTypes::Back] = ConnectionLibraryEntry::DiaphragmLoadType::DontApply;
         }
         else
         {
...

      {
         ...
         if (tmp == _T("ApplyAtBearingCenterline"))
         {
            m_DiaphragmLoadType[pgsTypes::Ahead] = ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtBearingCenterline;
         }
         else if (tmp == _T("ApplyAtSpecifiedLocation"))
         {
            m_DiaphragmLoadType[pgsTypes::Ahead] = ConnectionLibraryEntry::DiaphragmLoadType::ApplyAtSpecifiedLocation;
         ...
         {
            m_DiaphragmLoadType[pgsTypes::Ahead] = ConnectionLibraryEntry::DiaphragmLoadType::DontApply;
         }
         else
         {
            hr = STRLOAD_E_INVALIDFORMAT;
...
HRESULT CPierData2::LoadOldPierData(Float64 version,IStructuredLoad* pStrLoad,std::shared_ptr<IEAFProgress> pProgress,const std::_tstring& strUnitName)
{
   // Input is in an old format (the format is ConnectionLibraryEntryuper before version 3.0 when we added ConnectionLibraryEntryplice)
